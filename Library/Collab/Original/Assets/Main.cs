...
public class Main : MonoBehaviour
{
    ...
    public List<Card> twoPair = new List<Card>();//list used for if three pair is true then checking the highest of the three(and removing the other pair)
    ...
    */
    public void PairChecker(List<Card> list)
    {
        ...
        if (pairNumberThree == true)
        {
            ...
            List<Card> weakestCard = new List<Card>();
            weakestCard.Add(firstPair);
            weakestCard.Add(secondPair);
            weakestCard.Add(thirdPair);
            weakestCard.OrderBy(x => x.straightNumber);
            int firstCardIndex = weakestCard.FirstOrDefault().straightNumber;
            
            twoPair.Add(firstPair);
            twoPair.Add(secondPair);
            twoPair.Add(thirdPair);

            twoPair.OrderBy(x=>x.straightNumber).ToList();   //todofix this
            twoPair.RemoveAt(firstCardIndex);
